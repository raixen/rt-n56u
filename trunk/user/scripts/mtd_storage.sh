...
# 搜寻的AP不联网则列入黑名单文件/tmp/apblack.txt的功能：【0】关闭，【1】启动
# 在【系统管理-控制台】输入执行【echo "" > /tmp/apblack.txt】即可以清空黑名单
apblack=0

connect_ap=100    # 检查是否连上AP的最大时长； 1=1秒， 2=2秒....
connect_net=30    # 检查是否联网时最大时长；1=3秒，2=6秒.... (信号较差时建议设置成50)
sig=10                  # 跳过信号低于sig值的AP
fenge='>'             # 定义参数分隔符，在密码或名字与此冲突时需要修改，注意:下面配置一同修改
...
# 下面是信号填写参考例子：（删除前面的注释#可生，建议复制一行重写）
